...
if __name__ == "__main__":
    ...
    total_days = 365
    consecutive_days = 34
    last_signin_time = time.time() - 86399  # 该值仅用于测试
    exp = (114514, 191800)
    ...
    if total_days or time.time() - last_signin_time > 86400:
    ...
    img_bytes = get_signin_img(
        ...
        rewards=[
            Reward(num=3200, ico=join(dirname(__file__), "imgs", "原石.png")),
            Reward(name="纠缠之缘", num=20, ico=join(dirname(__file__), "imgs", "纠缠之缘.png")),
        ],
        font_path=font_path,
    )
